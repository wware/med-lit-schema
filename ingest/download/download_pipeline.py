...
Examples:
  ...
  python download_pipeline.py --pmc-id-file ids.txt --skip-existing
    ...
    input_group.add_argument(
        "--pmc-ids",
        nargs="+",
        help="PMC IDs to download (e.g., PMC123456 PMC234567)"
    )
    input_group.add_argument(
        "--pmc-id-file",
        type=str,
        help="File containing PMC IDs (one per line)"
    )
    input_group.add_argument(
        "--search",
        type=str,
        help="PubMed search query (downloads matching PMC articles)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="pmc_xmls",
        help="Output directory for XML files (default: pmc_xmls)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Maximum search results to download (default: 100)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="NCBI API key (increases rate limit from 3 to 10 req/sec)"
    )
    parser.add_argument(
        "--skip-existing",
        action="store_true",
        help="Skip PMC IDs that already have XML files"
    )
