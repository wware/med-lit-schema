...
from med_lit_schema.entity import (
    ...
    PaperMetadata, ExtractionProvenance, ExtractionPipelineInfo, ExecutionInfo
)
from med_lit_schema.relationship import create_relationship
from med_lit_schema.base import PredicateType, EntityReference


def create_fake_pmc_xml(paper_id: str, title: str, abstract: str, authors: list[str], 
                        journal: str, pub_date: str, pmid: str = None, doi: str = None) -> str:
...
def test_relationships():
    ...
    try:
        ...
        all_treats = storage.relationships.find_relationships(
            predicate="treats"
        )
        print(f"   ✓ Found {len(all_treats)} 'treats' relationships")
        
        breast_cancer_rels = storage.relationships.find_relationships(
            object_id="C0006142"
        )
